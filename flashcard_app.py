...
# Helper to auto-play audio via HTML (workaround for Streamlit's media player)
...
with tab1:
    ...
    if not st.session_state.flipped:
        ...
        audio_file = speak(f"How do you say {english}?", lang="en")
        st.markdown(autoplay_audio(audio_file), unsafe_allow_html=True)
    else:
        st.markdown(f'<div style="{card_style + card_back_style}">{croatian}</div>', unsafe_allow_html=True)
        audio_file = speak(croatian, lang="hr")
        st.markdown(autoplay_audio(audio_file), unsafe_allow_html=True)
    ...
    with col1:
        if st.button("🔁 Repeat"):
            audio_file1 = speak(f"How do you say {english}?", lang="en")
            audio_file2 = speak(croatian, lang="hr")
            st.markdown(autoplay_audio(audio_file1), unsafe_allow_html=True)
            st.markdown(autoplay_audio(audio_file2), unsafe_allow_html=True)
    with col2:
        if st.button("🪞 Flip"):
